...
def main():
    ...
    pd.DataFrame(execution_time_list).to_csv(
        os.path.join(result_directory, "dyndom_simulation_execution_time.csv"),
        index=False,
    )
...
def make_dyndom_command_files(df, result_directory):
    ...
    Parallel(n_jobs=os.cpu_count())(
        delayed(make_dyndom_command_file)(
            row["p_pdb"], row["q_pdb"], result_directory
        )
...
def run_dyndom(command_file_path):
    command = f"./DynDom {command_file_path}"
...
def run_all_dyndom(result_directory):
    ...
    results = Parallel(n_jobs=os.cpu_count())(
        delayed(run_dyndom_with_print)(command_file) for command_file in tqdm(command_files, total=len(command_files))
    )
    return results


def run_dyndom_with_print(command_file):
    print(f"Running Dyndom using command file {command_file}")
